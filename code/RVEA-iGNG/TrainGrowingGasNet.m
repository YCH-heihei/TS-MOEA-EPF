...
% This function is written by Qiqi Liu
    % V: 上一代用以指导种群优化的参考向量。
    % temp1: 当前所有的非被支配对应的目标向量。
    % net: GNG网络。
    % scale: 用于缩放的跨度。
    % params: 网络更新参数。
    % Problem
    % wholeObj: 存档中的目标向量。
    % genFlag: 是否需要产生参考向量的标记。
    % zmin
    %% Parameters
    N = params.N;
    MaxIt = params.MaxIt;
    ...
    E = net.E; % 累积误差。
    C = net.C; % 边。
    nx = net.nx; % 总的已用于更新GNG的节点数。
    ageSumBefore = net.ageSumBefore;% 更新前的age总和。
    flag = net.flag; % 用于判断节点是否超过阈值。
    
    % select the corner solutions using GNG
    output=[];
    fm =[]; 
    C = net.C; % 记录节点之间是否存在邻边。
    w= net.w; % 节点。
    for i = 1:size(w,1)
        neighbor = find(C(i,:)==1);
        ...
        if ageSum(i,:) == ageSumBefore(i,:)
            flag(i,:) = flag(i,:) + 1; % 更新无效标记(许久未更新的节点)。
        end
    ...
    if gen <= round(0.9*maxgen)
        ...
        maxIter = 1;
        maxPZ = maxN;
        if size(w,1) == round(maxN*N) % 正好处于节点数上界(1.5*N)
            %      PlotResults(w, C)
            % plot(w(:,1),w(:,2),'k*')
            % hold on
            % plot(temp1(:,1),temp1(:,2),'rs')
            [~,rankFlag] = sort(flag,'descend'); % 依据flag对应的无效标记删除最无效的maxN*N-N个。
        ...
        if size(w,1) == round(maxN*N)
            maxPZ = 1; % 什么意思哦？还需要产生？节点数都到1.5倍了。
            maxIter = 0; % 但又不训练。
            genFlag = gen; % 将存档的参考向量加入。
        end
    end
    % plot(w(:,1),w(:,2),'k*')
    ...
    if isempty(genFlag)
        for iter = 1:maxIter
            for kk = 1:size(temp1,1) %? 使用的不是存档，而是当前的非被支配个体。。。。
                nx = nx + 1;
                x = temp1(kk,:);
                w = w./sum(w,2);
                d = pdist2(x, w);
    ...
    if isempty(genFlag)
        ...
        V = w;
        V = V.*repmat(scale,size(V,1),1); % 将其还原回缩放前的位置。
    end




    % genFlag在节点数刚好为1.5倍种群规模时执行。
    if gen == genFlag
        N = size(wholeObj,1);
        % 标准化
        wholeObj1 = (wholeObj - repmat(zmin,N,1)); % 当前存档对应的目标向量，可能这是这个算法在VNT2上表现也较好的原因。
        wholeObj = wholeObj1./scale; % 怎么这个又缩放了，而w又乘以scale?
        temp2 = wholeObj1./sum(wholeObj1,2); 


        output = [];% 用来记录边界点，边界点是指与其存在连边的其它节点相比，其拥有最大值，或是最小值。
        for i = 1:size(w,1)
            neighbor = find(C(i,:)==1);
            [~,minInd] = min(w([i neighbor],:),[],1);
            [~,maxInd] = max(w([i neighbor],:),[],1);
