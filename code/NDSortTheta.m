function [FrontNo,MaxFNo] = NDSortTheta(varargin)
...
%--------------------------------------------------------------------------
    ...
    if M < 3 || N < 500
        ...
        [FrontNo,MaxFNo] = ENS_SS(PopObj,nSort,fuzzy);
    else
        % Use tree-based efficient non-dominated sort (T-ENS)
        [FrontNo,MaxFNo] = T_ENS(PopObj,nSort,fuzzy);
    end
end

function [FrontNo,MaxFNo] = ENS_SS(PopObj,nSort,fuzzy)
    originPopObj=PopObj;
    PopObj=ceil(PopObj./fuzzy);
    ...
    needToDistinguish=find(Table>=2);
    FrontNo = FrontNo(:,Loc);
    for i=1:length(needToDistinguish)
        current=find(Loc==needToDistinguish(i));
        [currenFno,~]=NDSort(originPopObj(current,:),length(current));
        FrontNo(current)=currenFno+min(FrontNo(current))-1;
    end
    while(sum(FrontNo<=MaxFNo)<nSort)
         MaxFNo=MaxFNo+1;
    end
end

function [FrontNo,MaxFNo] = T_ENS(PopObj,nSort,fuzzy)
    originPopObj=PopObj;
    PopObj=ceil(PopObj./fuzzy);
    [PopObj,~,Loc] = unique(PopObj,'rows');
    Table     = hist(Loc,1:max(Loc));
