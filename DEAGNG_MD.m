...
%                      AddToAS=offspring(FrontNo(childIndex)==1);
                      ...
                      tempMax=max(temp,[],2);
                      cosineToLine=pdist2(temp,ones(1,M),'cosine').*(tempMax);
                      [~,maxIndex]=max(cosineToLine);
                      cosineToLine(maxIndex)=[];
                      notInMaxIndex=setdiff(1:size(temp,1),maxIndex);
                      minCosDis=min(pdist2(temp(notInMaxIndex,:),temp(maxIndex(end),:),'cosine').*(tempMax(notInMaxIndex)),cosineToLine);
                %       minCosDis=pdist2(temp(notInMaxIndex,:),temp(maxIndex(end),:),'cosine')+cosineToLine;
                      while length(maxIndex)<M && length(notInMaxIndex)>0
                          ...
                          minCosDis=min(minCosDis,pdist2(temp(notInMaxIndex,:),temp(maxIndex(end),:),'cosine').*(tempMax(notInMaxIndex)));
                      end
                     
                     ExtremeVectors=temp(maxIndex,:);
%                      if(size(GNGnet.NodeP,1)>M)
%                          [~,EMIndex]=max(1-pdist2(ExtremeVectors,GNGnet.NodeS,'cosine'),[],2);
%                          EM=GNGnet.NodeS(EMIndex,:);
%                      else
% %                          EM=eye(M)+10.^-6;
%                         EM=ExtremeVectors;
%                      end
%                      q=0.02;
%                      EM=(1-q)*ExtremeVectors+q*(EM);
%                      EM=EM./(repmat(sum(EM,2),1,size(EM,1)));
%                    
                     %% 方案3
                     q=0.02;
                     EM=ExtremeVectors;
                     EM=EM-q*(mean(EM,1)-EM); 
                     EM(EM>1)=1;
                     EM(EM<0)=0;
                     EM=EM./(sum(EM,2));
                     %% 方案3
...
% %          expandRatio=1.1;
%             %          [~,oderOfM]=max(EM,[],1);
%             %          for i=1:length(oderOfM)
%                      EM=(EM.^(1.05));
%             %          for i=1:size(ExtremeVectors)
%             % %              [~,expandedIndex]=max(EM(oderOfM(i),:));
%             % %              EM(oderOfM(i),i)=EM(oderOfM(i),i)*expandRatio;
%             % %                          EM(i,expandedIndex)=EM(i,expandedIndex)+expandRatio;
%             % %                          otherIndex=[1:expandedIndex-1,expandedIndex+1:size(EM,2)];
%             % %                          sumOfOther=sum(EM(i,otherIndex));
%             % %                          EM(i,otherIndex)=(sumOfOther-0.01)/(sumOfOther)*EM(i,otherIndex);
%             %          end
%                       EM=EM./sum(EM);
                     %% 方案2
