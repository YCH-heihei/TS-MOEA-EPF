function PopObj = PF18OfZCAT(PopDec)
...
%   此处显示详细说明
    ...
    PopObj=zeros(N,M);
    %% 一维的流形
    index=all(PopDec<=0.4,2) | all(PopDec>=0.6,2); 
%      index=all(PopDec<=0.4 |PopDec>=0.6,2);
    ...
    PopObj(~index,M)=1/2*(sum((0.5-PopDec(~index,:)).^5,2)/(2*(M-1)*(0.5).^5)+1);
