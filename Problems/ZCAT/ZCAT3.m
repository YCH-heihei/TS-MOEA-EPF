...
%             PopObj=PopObj.*((1:M).^2);
            ...
            if(obj.complicatedParetoSet==0)
                ...
                PopDec(:,m+1:n)=PopDec(:,m+1:n)-g2(PopDec(:,1:m),n);
            end
...
%                         PopObj(:,i)=PopObj(:,i)+i^2*Z2(PopDec(:,m+find(indexOfM==i)),obj.K);
                    else
        ...
        function R = GetOptimum(obj,N)
         % the sampled points 3-objective ZCAT  as follow:
            if(obj.M==3||obj.M==2)
                ...
                tempPath=strcat(datePath,'\','ZCAT3','\','ZCAT3','_M',num2str(obj.M),'_Num',num2str(N),'.mat');
                load(tempPath);
                R=opt;
            else
                R=[];
