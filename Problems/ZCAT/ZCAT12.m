...
%             obj.K=obj.ParameterSet(3);
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            PopObj=PF12OfZCAT(PopDec(:,1:m),3);
        ...
        function R = GetOptimum(obj,N)
         % the sampled points 3-objective ZCAT  as follow:
            if(obj.M==3||obj.M==2)
                ...
                tempPath=strcat(datePath,'\','ZCAT12','\','ZCAT12','_M',num2str(obj.M),'_Num',num2str(N),'.mat');
                load(tempPath);
                R=opt;
            else
                R=[];
...
%                 load(tempPath);
                x=0:0.05:1;
                [X,Y]=meshgrid(x);
                opt=PF12OfZCAT([X(:),Y(:)],3);
                 [Fno,~]=NDSort(opt,1);
%                 index=find((x<0.244&&x>0.240)||x<0.259&&x>0.240);
