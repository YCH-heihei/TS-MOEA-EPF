...
%             PopObj=PopObj.*((1:M).^2);
            ...
            if(obj.complicatedParetoSet==0)
                ...
                PopDec(:,m+1:n)=PopDec(:,m+1:n)-g3(PopDec(:,1:m),n);
            end
        ...
        function R = GetOptimum(obj,N)
         % the sampled points 3-objective ZCAT  as follow:
            if(obj.M==3||obj.M==2)
                ...
                tempPath=strcat(datePath,'\','ZCAT20','\','ZCAT20','_M',num2str(obj.M),'_Num',num2str(N),'.mat');
                load(tempPath);
                R=opt;
            else
                R=[];
...
%                 opt=opt(orderIndex,:);
                ...
                %% 单独针对这个问题。
                % 一种网格化的方法，就是初始化一个固定格子数的网络(X-Y轴)，接着对每个点判断其所属格子，无结点的格子初始为NaN。
%                 R_X=zeros(100,100)+nan;
%                 R_Z=zeors(100,100)+nan;
%                 R_Y
%                 for i=1:size(opt,1)
%                     
%                 end 
                R_X=reshape(opt(:,1),num,[]);
                R_Y=reshape(opt(:,2),num,[]);
                R_Z=reshape(opt(:,3),num,[]);
                R={R_X,R_Y,R_Z};
            elseif(obj.M==2)
                x=0:0.01:1;
                opt=PF20OfZCAT(x');
