...
%             PopObj=PopObj.*((1:M).^2);
            ...
            if(obj.complicatedParetoSet==0)
                ...
                PopDec(:,m+1:n)=PopDec(:,m+1:n)-g1(PopDec(:,1:m),n);
            end
...
%                         PopObj(:,i)=PopObj(:,i)+i^2*Z1(PopDec(:,m+find(indexOfM==i)),obj.K);
        ...
        function R = GetOptimum(obj,N)
         % the sampled points 3-objective ZCAT  as follow:
            if(obj.M==3||obj.M==2)
                ...
                tempPath=strcat(datePath,'\','ZCAT17','\','ZCAT17','_M',num2str(obj.M),'_Num',num2str(N),'.mat');
                load(tempPath);
                R=opt;
            else
                R=[];
        ...
        function R = GetPF(obj)
            if(obj.M==3)
%                 datePath='.\Problems\Multi-objective optimization\ZCAT\ZCAT1';
...
%                 part1Index=opt(:,1)<0.55 & opt(:,2)<2.2;
%                 part2Index=opt(:,1)<0.55 & opt(:,2)>=2.2;
%                 part3Index=opt(:,1)>=0.55 & opt(:,2)<2.2;
%                 part4Index=opt(:,1)>=0.55 & opt(:,2)>=2.2;
%                 expandNum=(num+1)^2-num^2;
%                 stepSize=floor(expandNum/4);
%                 tempOpt=[];
%                 tempOpt=opt(part1Index,:);
%                 opt(end+1:end+stepSize,:)=zeros(stepSize,obj.M)+nan;
%                 opt=opt(part2Index,:);
%                 opt(end+1:end+stepSize,:)=zeros(stepSize,obj.M)+nan;
%                 opt1=opt(part1Index,:);
%                 opt2=opt(~part1Index,:);
